...
void time_handler() {
    ...
    auto it = timer_heap.top();
    if (it == nullptr) {
        alarm(TIMESLOT);
    } else {
        alarm(it->expire - time(NULL));
    }
}
    ...
    while (!stop_server) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if (sockfd == listenfd) {
                ...
                heapTimer* timer = users[sockfd].timer;
                if (ret < 0 || ret == 0) {
                    if (errno != EAGAIN || ret == 0) {     // 发生读错误或关闭连接
                        cb_func(&users[sockfd]);
                        if (timer) {
                            timer_heap.delTimer(timer);
                        }
                    }
                } else {
                    if (timer) {
                        heapTimer* Newtimer = new heapTimer(3 * TIMESLOT);
                        Newtimer->cb_func = timer->cb_func;
                        Newtimer->userData = timer->userData;
                        timer_heap.delTimer(timer);
                        timer_heap.addTimer(Newtimer);
