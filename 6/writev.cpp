...
int main(int argc, char* argv[]) {
    ...
    int connfd = accept(sock, (sockaddr*)&client, &client_addrlenght);
    if (connfd < 0) {
        printf("errno is %d\n", errno);
    } else {
        ...
        if (valid) {
            ...
            len += ret;
            ret = snprintf(header_buf + len, BUFFER_SIZE - 1, "Content-Length: %ld\r\n", file_stat.st_size);
            len += ret;
            ret = snprintf(header_buf + len, BUFFER_SIZE - 1 - len, "%s", "\r\n");
            /* 利用writev将header_buf 和 file_buf 的内容一并写出 */
            iovec iv[2];
